...
check "stock QCA6750 selector disabled" grep -q '^# CONFIG_CNSS_QCA6750 is not set
check "MI memory config" grep -q '^CONFIG_MI_MEMORY_SYSFS=m' "$CFG"
check "MI hardware id module selector" grep -q '^CONFIG_MI_HARDWARE_ID=m' "$CFG"
check "stock MI hardware id is module" grep -q '^CONFIG_MI_HARDWARE_ID=m
...
exit "$fail"
 "$STOCK_CFG"
check "stock UFSGKI marker" grep -q '^CONFIG_UFSGKI=y
check "MI memory config" grep -q '^CONFIG_MI_MEMORY_SYSFS=m' "$CFG"
...
exit "$fail"
 "$STOCK_CFG"
check "stock UFS write booster marker" grep -q '^CONFIG_UFS_WB=y
check "MI memory config" grep -q '^CONFIG_MI_MEMORY_SYSFS=m' "$CFG"
check "MI hardware id is built-in" grep -q '^CONFIG_MI_HARDWARE_ID=y' "$CFG"
check "MI thermal interface is module" grep -q '^CONFIG_MI_THERMAL_INTERFACE=m' "$CFG"
check "USB DTP is module" grep -q '^CONFIG_USB_F_DTP=m' "$CFG"
...
exit "$fail"
 "$STOCK_CFG"
check "stock Xiaomi UFS FFU marker" grep -q '^CONFIG_MI_UFS_FFU=y
check "MI memory config" grep -q '^CONFIG_MI_MEMORY_SYSFS=m' "$CFG"
check "MI hardware id is built-in" grep -q '^CONFIG_MI_HARDWARE_ID=y' "$CFG"
check "MI thermal interface is module" grep -q '^CONFIG_MI_THERMAL_INTERFACE=m' "$CFG"
check "USB DTP is module" grep -q '^CONFIG_USB_F_DTP=m' "$CFG"
