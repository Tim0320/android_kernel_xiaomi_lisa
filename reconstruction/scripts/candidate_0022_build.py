...
def overlay_known_good():
    ...
    (ROOT/"candidate-0022-overlay.txt").write_text(
...
def patch_yupik():
    ...
    new="\t/* Lisa Candidate 0022 diagnostic: never touch inaccessible IPA QoS MMIO. */\n\t.qosbox = NULL,"
    ...
    marker_new=marker+'\n\tif (desc == &yupik_aggre2_noc)\n\t\tdev_info(&pdev->dev, "Lisa Candidate 0022: qxm_ipa QoS fully disabled\\n");\n'
    ...
    if "Lisa Candidate 0022: qxm_ipa QoS fully disabled" not in out:
        raise SystemExit("Candidate 0022 runtime marker missing")

    (ROOT/"candidate-0022-root-cause.txt").write_text(
        ...
        "candidate_0022_control=retain qxm_ipa.qosbox=NULL; overlay known-good 5.4.302 secure/UFS + zram paths\n"
...
def repack():
    ...
    kernel=(ROOT/"candidate-0022-Image").read_bytes()
    ...
    (ROOT/"candidate-0022-repack.txt").write_text(
        f"candidate_0022_image_sha256={sha256(kernel)}\n"
        f"candidate_0022_image_bytes={len(kernel)}\n"
        f"stock_kernel_region_bytes={ksz}\n"
        f"kernel_zero_pad_bytes={ksz-len(kernel)}\n"
        f"candidate_0022_boot_sha256={sha256(out)}\n"
...
def main():
    ...
    (ROOT/"candidate-0022.config").write_bytes((OUT/".config").read_bytes())
    ...
    (ROOT/"candidate-0022-Image").write_bytes(built.read_bytes())
    ik=subprocess.check_output([str(KERNEL/"scripts/extract-ikconfig"), str(built)])
    (ROOT/"candidate-0022.ikconfig").write_bytes(ik)
    if TARGET_RELEASE not in built.read_bytes():
        raise SystemExit("target release missing from Candidate 0022 Image")
    repack()

    manifest=(
        "candidate=Lisa Candidate 0022 known-good secure/UFS/zram alignment\n"
        f"kernel_source_sha={SOURCE_SHA}\n"
        f"candidate_0018_source_image_sha256={KNOWN_IMAGE_SHA}\n"
        f"candidate_0022_image_sha256={sha256(ROOT/'candidate-0022-Image')}\n"
        f"candidate_0022_boot_sha256={sha256(ROOT/'boot.img')}\n"
        "companion_base=reconstruction/stock/stock-Image-3.09\n"
        "mutation=retain qxm_ipa QoS hard-disable + align qseecom/qcom_scm/smcinvoke/UFS/zram to known-good 5.4.302\n"
        "fault_register_offset=0x10008\n"
        "LISA_CANDIDATE_0022_FINAL_GATE=PASS\n"
    )
    (ROOT/"candidate-0022-manifest.txt").write_text(manifest)
    print(manifest)
